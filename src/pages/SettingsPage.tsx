...
export function SettingsPage() {
  ...
  return (
    <div className="settings-page">
      ...
      <section className="settings-section"><div className="section-heading"><div><h2>Antigravity analyst</h2><p>Authenticated through your installed Google AI Pro subscription.</p></div>{aiStatus.available ? <CheckCircle size={21} weight="fill" /> : <WarningCircle size={21} weight="fill" />}</div><div className={`ai-connection ${aiStatus.state}`}><span className="analyst-dot" /><div><strong>{aiStatus.available ? `${aiStatus.provider} connected` : 'Antigravity CLI unavailable'}</strong><p>{aiStatus.access}</p><small>Optional answer reviews: {aiStatus.observerModel} · Interval and weekly reports: {aiStatus.reportModel}</small>{aiStatus.lastError && <small className="error-copy">{aiStatus.lastError}</small>}</div></div><div className="privacy-note"><ShieldCheck size={19} /><p>No key is embedded in the app. The local server invokes <code>agy</code>, which uses your existing Google OAuth session. Gemini runs after completed sets and mocks, or when you explicitly request an answer review. Raw evidence is saved first.</p></div></section>
