...
function newMock(): ActiveMock {
  const seed = Math.floor(Date.now() / 1000)
  const modules = buildInitialMock(seed)
  return { id: crypto.randomUUID(), seed, modules, moduleIndex: 0, questionIndex: 0, answers: {}, flags: [], remaining: modules[0].durationSeconds, stage: 'intro', startedAt: new Date().toISOString() }
}
...
export function MockRunner() {
    ...
    const timer = window.setInterval(() => {
      setMock((value) => {
        if (value.remaining <= 1) {
          ...
          return { ...value, remaining: 0, stage: 'review' }
        }
  ...
  const goQuestion = (index: number) => {
    setMock((state) => ({ ...state, questionIndex: Math.max(0, Math.min(module.questions.length - 1, index)), stage: 'question' }))
  ...
  const finishModule = async () => {
    ...
    if (module.id === 'rw-1') {
      ...
      const remaining = buildRemainingMock(mock.seed, route, module.questions)
